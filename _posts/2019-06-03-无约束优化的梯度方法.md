...
* 下面的证明会看到，选取步长使得 $|1-\eta \lambda_{n}(Q)|=|1-\eta \lambda_{1}(Q)|$
...
* Analysis: 想比较每次迭代后得到的结果和最优解的距离，先带入第t+1次GD迭代rule，
  ...
  $$x^{t+1}-x^{*}=x^{t}-x^{*}-\eta_{t} \nabla f\left(x^{t}\right)=\left(\bm{I}-\eta_{t} \bm{Q}\right)\left(\bm{x}^{t}-\bm{x}^{*}\right)$$

  由Cauchy-Schwarz不等式得到此次距离和上次距离的关系，一个upper bound:

  $$\left\|x^{t+1}-x^{*}\right\|_{2} \leq\left\|\bm{I}-\eta_{t} \bm{Q}\right\|\left\|\bm{x}^{t}-\bm{x}^{*}\right\|_{2}.$$

  现在要做的事情就是找到最优的 $\eta_t$ 使得每一步迭代都尽可能使得下一步距离更接近，
  即最小化谱范数 $\|\bm{I}-\eta \bm{Q}\|$。
  假如已经知道$Q$的特征值 $\lambda_1\ge...\ge\lambda_n\ge0$，
  那么$\bm{I}-\eta \bm{Q}$的特征值$1-\eta\lambda_1\le...\le1-\eta\lambda_n$，
  但如果没有具体例子，无法判断哪一个绝对值最大。
  但是谱范数找的是绝对值最大的特征值的绝对值，因此可以确定
  
  $$\|\bm{I}-\eta \bm{Q}\|=\max \left\{\left|1-\eta_{t} \lambda_{1}(\bm{Q})\right|,\left|1-\eta_{t} \lambda_{n}(\bm{Q})\right|\right\}$$
  
  那么由此我们有

  $$\eta = \arg\min  \max \left\{\left|1-\eta_{t} \lambda_{1}(\bm{Q})\right|,\left|1-\eta_{t} \lambda_{n}(\bm{Q})\right|\right\} =\frac{2}{\lambda_{1}(Q)+\lambda_{n}(Q)}$$

  此时
  
  $$\|I-\eta Q\|=1-\frac{2 \lambda_{n}(\bm{Q})}{\lambda_{1}(\bm{Q})+\lambda_{n}(\bm{Q})}=\frac{\lambda_{1}(\bm{Q})-\lambda_{n}(\bm{Q})}{\lambda_{1}(\bm{Q})+\lambda_{n}(\bm{Q})}$$
  
  最后解释$\eta$的最优解如何求到的：
  根据$\left|1-\eta_{t} \lambda_{1}(\bm{Q})\right|,\left|1-\eta_{t} \lambda_{n}(\bm{Q})\right|$的大小关系对$\eta$的取值和目标函数的取值进行分类讨论，

  + 最小的特征值大于等于0： 
     $0 \leq 1-\eta \lambda_{1}\Rightarrow \eta \leq \frac{1}{\lambda_{1}}$，
     此时$\|\bm{I}-\eta \bm{Q}\|=\left|1-\eta_{t} \lambda_{n}(\bm{Q})\right|\geqslant \frac{\lambda_{1}-\lambda_{n}}{\lambda_{1}}$
  + 最大的特征值小于等于0:
    $1- \lambda_{n} \leq 0 \Rightarrow \eta \geqslant \frac{1} {\lambda_{n}}$，此时$\|\bm{I}-\eta \bm{Q}\|=\left|1-\eta_{t} \lambda_{1}(\bm{Q})\right|\geqslant \frac{\lambda_{1}-\lambda_{n}}{\lambda_{1}}$
  + 最小的小于0,最大的大于0： 
    $1-\eta \lambda_{1}<0, \; 1-\eta \lambda_{n}>0 \Rightarrow \frac{1} {\lambda_{1}} \leqslant\eta \leqslant \frac{1}{\lambda_{n}}$. 该情况又分为两种： $\frac{1} {\lambda_{1}} <\eta \leqslant \frac{2}{\lambda_1+\lambda_{n}}$, 和 $\frac{2} {\lambda_{1}+\lambda_n} \leqslant\eta < \frac{1}{\lambda_{n}}$，最终可得$\eta = \frac{2} {\lambda_{1}+\lambda_n}$时 $\|\bm{I}-\eta \bm{Q}\|$取到最小 $\frac{\lambda_{1}(\bm{Q})-\lambda_{n}(\bm{Q})}{\lambda_{1}(\bm{Q})+\lambda_{n}(\bm{Q})}$。
    这也是三种情况中的最小情况。
...
$$
\eta_{t}=\arg \min _{\eta \geq 0} f\left(\bm{x}^{t}-\eta \nabla f\left(\bm{x}^{t}\right)\right)
$$
...
$$
f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right) \leq\left(\frac{\lambda_{1}(\bm{Q})-\lambda_{n}(\bm{Q})}{\lambda_{1}(\bm{Q})+\lambda_{n}(\bm{Q})}\right)^{2 t}\left(f\left(\bm{x}^{0}\right)-f\left(\bm{x}^{*}\right)\right)
$$

* 与常数步长的收敛分析不同，此处用目标值说明收敛速度。
* 收敛速度为线性收敛 linear convergence，和常数步长的情况相似。
* Analysis: 
  为方便，设 $\bm{g}^{t}=\nabla f\left(\bm{x}^{t}\right)=\bm{Q}\left(\bm{x}^{t}-\bm{x}^{*}\right)$，根据 exact line search rule 可以得到 $\eta_{t}=\frac{\bm{g}^{t \top} \bm{g}^{t}}{\bm{g}^{t^{\top}} \bm{Q} \bm{g}^{t}}$. 代入第 t+1 次的结果，找出与第 t 次的关系：

  $$
  \begin{aligned} 
  f\left(\bm{x}^{t+1}\right) &=\frac{1}{2}\left(\bm{x}^{t}-\eta_{t} \bm{g}^{t}-\bm{x}^{*}\right)^{\top} \bm{Q}\left(\bm{x}^{t}-\eta_{t} \bm{g}^{t}-\bm{x}^{*}\right) \\ &=\frac{1}{2}\left(\bm{x}^{t}-\bm{x}^{*}\right)^{\top} \bm{Q}\left(\bm{x}^{t}-\bm{x}^{*}\right)-\eta_{t}\left\|\bm{g}^{t}\right\|_{2}^{2}+\frac{\eta_{t}^{2}}{2} \bm{g}^{t \top} \bm{Q} \bm{g}^{t} \\ &=\frac{1}{2}\left(\bm{x}^{t}-\bm{x}^{*}\right)^{\top} \bm{Q}\left(\bm{x}^{t}-\bm{x}^{*}\right)-\frac{\left\|\bm{g}^{t}\right\|_{2}^{4}}{2 \bm{g}^{t \top} \bm{Q} \bm{g}^{t}} \\ &=\left(1-\frac{\left\|\bm{g}^{t}\right\|_{2}^{4}}{\left(\bm{g}^{t \top} \bm{Q} \bm{g}^{t}\right)\left(\bm{g}^{t \top} \bm{Q}^{-1} \bm{g}^{t}\right)}\right) f\left(\bm{x}^{t}\right) 
  \end{aligned}
  $$

  最后一个等号用到了 $f\left(x^{t}\right)=\frac{1}{2}\left(x^{t}-x^{*}\right)^{\top} Q\left(x^{t}-x^{*}\right)=\frac{1}{2} g^{t \top} Q^{-1} g^{t}$.
  由 Kantorovich 不等式 （下降方法收敛性研究的核心）$\frac{\|\bm{y}\|_{2}^{4}}{\left(\bm{y}^{\top} \bm{Q} \bm{y}\right)\left(\bm{y}^{\top} \bm{Q}^{-1} \bm{y}\right)} \geq \frac{4 \lambda_{1}(\bm{Q}) \lambda_{n}(\bm{Q})}{\left(\lambda_{1}(\bm{Q})+\lambda_{n}(\bm{Q})\right)^{2}}$，代入得到

  $$
  \begin{aligned} f\left(\bm{x}^{t+1}\right) & \leq\left(1-\frac{4 \lambda_{1}(\bm{Q}) \lambda_{n}(\bm{Q})}{\left(\lambda_{1}(\bm{Q})+\lambda_{n}(\bm{Q})\right)^{2}}\right) f\left(\bm{x}^{t}\right) \\ &=\left(\frac{\lambda_{1}(\bm{Q})-\lambda_{n}(\bm{Q})}{\lambda_{1}(\bm{Q})+\lambda_{n}(\bm{Q})}\right)^{2} f\left(\bm{x}^{t}\right) \end{aligned}
 $$

 由于已知 $f\left(\bm{x}^{*}\right)=\min _{\bm{x}} f(\bm{x})=0$，收敛性得证。
...
$$
\mathbf{0} \preceq \mu \bm{I} \preceq \nabla^{2} f(\bm{x}) \preceq L \bm{I}, \quad \forall \bm{x}
$$
...
* 与二次函数情况的对比，步长 $\eta=\frac{2}{\mu+L}$   V.S.  $\eta=\frac{2}{\lambda_{1}(\bm{Q})+\lambda_{n}(\bm{Q})}$;  收缩比例 $\frac{\kappa-1}{\kappa+1}$  V.S.  $\frac{\lambda_{1}(Q)-\lambda_{n}(Q)}{\lambda_{1}(Q)+\lambda_{n}(Q)}$
...
* 依照smoothness的定义，以及 $\nabla f(x^*)=0$, 可得
  $$
  f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right) \leq \frac{L}{2}\left(\frac{\kappa-1}{\kappa+1}\right)^{2 t}\left\|\bm{x}^{0}-\bm{x}^{*}\right\|_{2}^{2}
  $$
* Analysis: 设 $g(\tau)=f(x_\tau)=f(x^t+\tau(x^*-x^t))$, 则 $\int_0^1 g''(\tau)d\tau = g'(1)-g'(0)$. 其中$\left\{x_{\tau}\right\}_{0 \leq \tau \leq 1}$ 可看成是 $x^t$ 到 $x^*$ 间的线段.
 ...
 $$\nabla f\left(\bm{x}^{t}\right)=\nabla f\left(\bm{x}^{t}\right)-\nabla f\left(\bm{x}^{*}\right)= g'(1)-g'(0) = \left(\int_{0}^{1} \nabla^{2} f\left(\bm{x}_{\tau}\right) \mathrm{d} \tau\right)\left(\bm{x}^{t}-\bm{x}^{*}\right)$$

$$
\begin{aligned}\left\|\bm{x}^{t+1}-\bm{x}^{*}\right\|_{2} &=\left\|\bm{x}^{t}-\bm{x}^{*}-\eta \nabla f\left(\bm{x}^{t}\right)\right\|_{2} \\ &=\left\|\left(\bm{I}-\eta \int_{0}^{1} \nabla^{2} f\left(\bm{x}_{\tau}\right) \mathrm{d} \tau\right)\left(\bm{x}^{t}-\bm{x}^{*}\right)\right\| \\ & \leq \sup _{0 \leq \tau \leq 1}\left\|\bm{I}-\eta \nabla^{2} f\left(\bm{x}_{\tau}\right)\right\|\left\|\bm{x}^{t}-\bm{x}^{*}\right\|_{2} \\ & \leq \frac{L-\mu}{L+\mu}\left\|\bm{x}^{t}-\bm{x}^{*}\right\|_{2} \end{aligned}
$$
...
$$
f\left(\bm{x}^{t}-\eta \nabla f\left(\bm{x}^{t}\right)\right)<f\left(\bm{x}^{t}\right)-\alpha \eta\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}
$$
...
这个值我们取目标迭代函数值的上限 $f\left(\bm{x}^{t}\right)-\alpha \eta\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}$ 与二阶近似的上限 $f\left(\bm{x}^{t}\right)-\eta\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}+\frac{L \eta^{2}}{2}\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}$ 相等的根。
这里二阶近似的上限用到了目标函数的光滑性 $L$-smoothness. 这是因为对于光滑函数，当 $\eta = \tilde{\eta}_t$ 时，$f\left(\bm{x}^{t}-\eta \nabla f\left(\bm{x}^{t}\right)\right)\le f\left(\bm{x}^{t}\right)-\eta\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}+\frac{L \eta^{2}}{2}\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}$.

$$
\begin{array}{l}{f\left(\bm{x}^{t}\right)-\alpha \eta\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}=f\left(\bm{x}^{t}\right)-\eta\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}+\frac{L \eta^{2}}{2}\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}} \\ 
...
$$
f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right) \leq\left(1-\min \left\{2 \mu \alpha, \frac{2 \beta \alpha \mu}{L}\right\}\right)^{t}\left(f\left(\bm{x}^{0}\right)-f\left(\bm{x}^{*}\right)\right)
$$
...
$$\frac{\partial \ell(\bm{\beta})}{\partial \bm{\beta}}=\sum_{i=1}^{N} \bm{x}_{i}\left(y_{i}-p\left(\bm{x}_{i} ; \bm{\beta}\right)\right)=\bm{0}$$

这实际上是 $p+1$ 个关于 $\bm{\beta}$ 的非线性方程。由于 $\bm{x}_i$ 的第一项为1，那么第一个非线性方程要求 $\sum_{i=1}^{N} y_{i}=\sum_{i=1}^{N} p\left(\bm{x}_i ; \bm{\beta}\right)$，即观察到分类为1的样本数目与期望的分类为1的数目一致（分类为0的样本数目因此也是一致的）。
进一步求其 Hessian 矩阵，
...
$$
\mu \bm{I} \preceq \nabla^{2} f(\bm{x}) \preceq L \bm{I}, \quad \forall \bm{x} \in \mathcal{B}_{0}
$$

where $\mathcal{B}_{0} :=\left\{\bm{x} :\left\|\bm{x}-\bm{x}^{*}\right\|_{2} \leq\left\|\bm{x}^{0}-\bm{x}^{*}\right\|_{2}\right\}$. Then **Theorem 1** continues to hold, i.e., if $\eta_{t} \equiv \eta=\frac{2}{\mu+L}$, then
...
* 对任意 $x^{t} \in \mathcal{B}_{0}$， 可得 $\left\|x^{t+1}-x^{*}\right\|_{2} \leq \frac{\kappa-1}{\kappa+1}\left\|x^{t}-x^{*}\right\|_{2}$，则有 $\bm{x}^{t+1} \in \mathcal{B}_{0}$，因此上述上限将在后来的迭代中继续成立。
* 回到逻辑回归的例子，local strong convexity 的参数表达式为
  
  $$
  \inf _{x :\left\|x-x^{*}\right\|_{2} \leq\left\|x^{0}-x^{*}\right\|_{2}} \lambda_{\min }\left( \sum_{i=1}^{N}  \frac{\exp \left( \beta^{\top} x_i \right)}{\left(1+\exp \left(\beta^{\top} x_i\right)\right)^{2}}  \bm{a}_{i} \bm{a}_{i}^{\top} \right)
  $$
...
$$
\left\langle\nabla f(\bm{x}), \bm{x}-\bm{x}^{*}\right\rangle \geq \frac{\mu}{2}\left\|\bm{x}-\bm{x}^{*}\right\|_{2}^{2}+\frac{1}{2 L}\|\nabla f(\bm{x})\|_{2}^{2}, \quad \forall \bm{x}
$$
...
$$
\|\nabla f(\bm{x})\|_{2}^{2} \geq 2 \mu\left(f(\bm{x})-f\left(\bm{x}^{*}\right)\right), \quad \forall \bm{x}
$$
...
$$
f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right) \leq\left(1-\frac{\mu}{L}\right)^{t}\left(f\left(\bm{x}^{0}\right)-f\left(\bm{x}^{*}\right)\right)
$$
...
$$
\nabla^{2} f(\bm{x})=\sum_{i=1}^{n} \bm{a}_{i} \bm{a}_{i}^{\top}
$$

当 $d>n$ 时不满秩。但大多数时候仍然满足 $f\left(\bm{x}^{*}\right)=0$ 以及 PL 条件，因此GD线性收敛。

**Fact 1** Suppose that $\bm{A}=\left[\bm{a}_{1}, \cdots, \bm{a}_{n}\right]^{\top} \in \mathbb{R}^{n \times d}$ has rank $n$, and that $\eta_{t} \equiv \eta=\frac{1}{\lambda_{\max }\left(A A^{\top}\right)}$. Then GD obeys 

$$
f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right) \leq\left(1-\frac{\lambda_{\min }\left(\bm{A} \bm{A}^{\top}\right)}{\lambda_{\max }\left(\bm{A} \bm{A}^{\top}\right)}\right)^{t}\left(f\left(\bm{x}^{0}\right)-f\left(\bm{x}^{*}\right)\right), \quad \forall t
$$

* 原问题的 Hessian $\nabla^{2} f(\bm{x})=\sum_{i=1}^{n} \bm{a}_{i} \bm{a}_{i}^{\top} = \bm{A}^{\top} \bm{A}\in \mathbb{R}^{d\times d}$
* 对 $\left\{\bm{a}_{i}\right\}$ 的假设温和
* 对 $\left\{y_{i}\right\}$ 没有要求
* 当有很多全局最优解时，对该over-parametrized 问题，GD 给出的结果有偏好，往往收敛到距离初始化 $x^0$ 最近的全局最优。
* **证明**：
  $\bm{A}^{\top} \bm{A}$ 和 $\bm{A} \bm{A}^{\top}$ 的特征值除0外相同（可以用SVD证明），因此最大的特征值相同。由于在Over-Parametrized 问题中 $f\left(x^{*}\right) = 0$ 因此只需要证明 $\lambda_{\min }\left(A A^{\top}\right)$ 就是 PL 条件中的 $\mu$ 即可，也就是$\|\nabla f(\bm{x})\|_{2}^{2} \geq 2 \lambda_{\min }\left(\bm{A} \bm{A}^{\top}\right) f(\bm{x})$. 如果该不等式成立，那么 Fact 1 得证。下面就证明这个不等式。
  令 $\bm{y}=\left[y_{i}\right]_{1 \leq i \leq n}$， 则 $\bm{x}^{*}=\bm{x}-\bm{A}^{\top}\left(\bm{A} \bm{A}^{\top}\right)^{-1}(\bm{A} \bm{x}-\bm{y}) = \arg \min _{A z=y}\|z-x\|_{2}$. 我们有

  $$
  \begin{aligned} 
  \nabla f(\bm{x}) &=\sum_{i} \bm{a}_{i}\left(\bm{a}_{i}^{\top} \bm{x}-y_{i}\right)=\sum_{i} \bm{a}_{i}\left(\bm{a}_{i}^{\top} \bm{x}-\bm{a}_{i}^{\top} \bm{x}^{*}\right) \\ 
  &=\left(\sum_{i} \bm{a}_{i} \bm{a}_{i}^{\top}\right)\left(\bm{x}-\bm{x}^{*}\right)=\bm{A}^{\top} \bm{A}\left(\bm{x}-\bm{x}^{*}\right) \\ 
  &=\bm{A}^{\top} \bm{A} \bm{A}^{\top}\left(\bm{A} \bm{A}^{\top}\right)^{-1}(\bm{A} \bm{x}-\bm{y}) \quad  \\ 
  &=\bm{A}^{\top}(\bm{A} \bm{x}-\bm{y}) 
  \end{aligned}
 $$
 ...
 $$
 \begin{aligned}
 \|\nabla f(\bm{x})\|_{2}^{2} &=(\bm{A} \bm{x}-\bm{y})^{\top} \bm{A} \bm{A}^{\top}(\bm{A} \bm{x}-\bm{y}) \\ 
 & \geq \lambda_{\min }\left(\bm{A} \bm{A}^{\top}\right)\|\bm{A} \bm{x}-\bm{y}\|_{2}^{2} \\ 
 &=2 \lambda_{\min }\left(\bm{A} \bm{A}^{\top}\right) f(\bm{x}) \quad \blacksquare
 \end{aligned} 
 $$
...
那么问题来了，不具备强凸性时，我们能保证目标函数值下降（i.e., $f\left(\bm{x}^{t+1}\right)<f\left(\bm{x}^{t}\right)$）吗？如何选择步长才能保证充分的下降呢？
关键思想： **majorization-minimization**, 给 $f(x)$ 找到简单的 majorizing function 然后优化这个替代函数。

由于光滑，

$$
\begin{aligned} f\left(\bm{x}^{t+1}\right)-f\left(\bm{x}^{t}\right) & \leq \nabla f\left(\bm{x}^{t}\right)^{\top}\left(\bm{x}^{t+1}-\bm{x}^{t}\right)+\frac{L}{2}\left\|\bm{x}^{t+1}-\bm{x}^{t}\right\|_{2}^{2} \\ &=-\eta_{t}\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}+\frac{\eta_{t}^{2} L}{2}\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2} \end{aligned}
$$

最后一个等号后面的就是由光滑得到的目标函数下降的 majorizing function，我们想要最大化每次迭代的下降程度的下限，就要最大化 $\left(\eta_{t}-\frac{\eta_{t}^{2} L}{2}\right)\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}$, 选择 $\eta_{t}=1 / L$ 取到最大，此时每次迭代至少使目标函数下降 $\frac{1}{2 L}\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}$. 我们就有了下面的 **Fact 2**.

**Fact 2** Suppose $f$ is L-smooth. Then GD with $\eta_{t}=1 / L$ obeys

$$
f\left(\bm{x}^{t+1}\right) \leq f\left(\bm{x}^{t}\right)-\frac{1}{2 L}\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}
$$

* 当步长足够小时，GD可以保证目标函数值下降。因为：令下降值下限  $\left(\eta_{t}-\frac{\eta_{t}^{2} L}{2}\right)\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}>0$ 可得 $0<\eta_t<2/L$.
* 并不依赖凸性！
...
实际上，可以进一步证明除非 $\bm{x}^{t}$ 已经是最优解，$\left\|x^{t}-x^{*}\right\|_{2}$ 是严格下降的。
...
* 证明： 
  ...
  &=\left\|x^{t}-x^{*}\right\|_{2}^{2}-2 \eta\left\langle\bm{x}^{t}-\bm{x}^{*}, \nabla f\left(\bm{x}^{t}\right)-\nabla f\left(\bm{x}^{*}\right)\right\rangle+\eta^{2}\left\|\nabla f\left(\bm{x}^{t}\right)-\nabla f\left(\bm{x}^{*}\right)\right\|_{2}^{2}
  \end{aligned}$$

  由于 smooth 且 convex， $\left\langle\bm{x}^{t}-\bm{x}^{*}, \nabla f\left(\bm{x}^{t}\right)-\nabla f\left(\bm{x}^{*}\right)\right\rangle \geq \frac{1}{L}\left\|\nabla f\left(\bm{x}^{t}\right)-\nabla f\left(\bm{x}^{*}\right)\right\|_{2}^{2}$, 又因为 $\eta_{t}=1 / L$， 我们有 $\left\|x^{t+1}-x^{*}\right\|_{2}^{2}\leq\left\|\bm{x}^{t}-\bm{x}^{*}\right\|_{2}^{2}-\eta^{2}\left\|\nabla f\left(\bm{x}^{t}\right)-\nabla f\left(\bm{x}^{*}\right)\right\|_{2}^{2}$， 其中 $\nabla f\left(\bm{x}^{*}\right)=0$. $\; \blacksquare$
...
$$
\begin{aligned} 
f\left(\bm{x}^{t+1}\right)-f\left(\bm{x}^{*}\right) & \leq f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right)-\frac{1}{2 L}\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2} \\ 
& \leq f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right)-\frac{\mu}{L}\left(f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right)\right) \\ 
&=\left(1-\frac{\mu}{L}\right)\left(f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right)\right) 
\end{aligned}
$$
...
$$
f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right) \leq \frac{2 L\left\|\bm{x}^{0}-\bm{x}^{*}\right\|_{2}^{2}}{t}
$$

where $x^*$ is any minimizer with optimal $f(x^*)$.

* 要达到 $\epsilon$-准确，需要 $O(1/\epsilon)$ 次迭代。线性收敛： $O(\log{\frac{1}{\epsilon}})$.
* 证明： 由 **Fact 2**， $f\left(\bm{x}^{t+1}\right)-f\left(\bm{x}^{t}\right) \leq-\frac{1}{2 L}\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}^{2}$. 
  为了递归的表示 $f\left(\bm{x}^{t}\right)$，我们将 $\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}$ 替换为 $f\left(\bm{x}^{t}\right)$ 的函数。
  为此，利用 convexity 得到 

  $$\begin{aligned} &f\left(\bm{x}^{*}\right)-f\left(\bm{x}^{t}\right) \geq \nabla f\left(\bm{x}^{t}\right)^{\top}\left(\bm{x}^{*}-\bm{x}^{t}\right) \geq-\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2}\left\|\bm{x}^{t}-\bm{x}^{*}\right\|_{2} \\
  &\Longrightarrow \quad\left\|\nabla f\left(\bm{x}^{t}\right)\right\|_{2} \geq \frac{f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right)}{\left\|\bm{x}^{t}-\bm{x}^{*}\right\|_{2}} \ge \frac{f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right)}{\left\|\bm{x}^{0}-\bm{x}^{*}\right\|_{2}} \end{aligned}$$

  令 $\Delta_{t} :=f\left(\bm{x}^{t}\right)-f\left(\bm{x}^{*}\right)$  可得 $\Delta_{t+1}-\Delta_{t} \leq-\frac{1}{2 L\left\|\bm{x}^{0}-\bm{x}^{*}\right\|_{2}^{2}} \Delta_{t}^{2}$ （代入Fact 2）
  下面用归纳法证明 $\Delta_{t} \leq \frac{b}{t} \quad$，其中 $b=2 L\left\|\bm{x}^{0}-\bm{x}^{*}\right\|_{2}^{2}$. 如果该不等式对 $t$ 成立，那么 $\Delta_{t+1} \leq \Delta_{t}-\Delta_{t}^2/b $ , 而这个不等式在 $\Delta_{t}<b/2$ 时增，我们让 $\Delta_{t} \leq \frac{b}{t}\le \frac{b}{2}$，那么函数始终对 $\Delta_{t}$ 是增的，也就是当 $t\ge 2$ 时函数对 $\Delta_{t}$ 增。因此有 $\Delta_{t+1} \leq \frac{b(t-1)}{t^{2}} \leq \frac{b}{t+1}$. $\; \blacksquare$
...
* in general, GD obeys 
  ...
  $$
  \min _{0 \leq k<t}\left\|\nabla f\left(\bm{x}^{k}\right)\right\|_{2} \leq \sqrt{\frac{2 L\left(f\left(\bm{x}^{0}\right)-f\left(\bm{x}^{*}\right)\right)}{t}}
  $$

* if $f$ is convex, then GD obeys
  
  $$
  \min _{t / 2 \leq k<t}\left\|\nabla f\left(\bm{x}^{k}\right)\right\|_{2} \leq \frac{4 L\left\|\bm{x}^{0}-\bm{x}^{*}\right\|_{2}}{t}
  $$
...
**证明**： 由 Fact 2 有 $\frac{1}{2 L}\left\|\nabla f\left(\bm{x}^{k}\right)\right\|_{2}^{2} \leq f\left(\bm{x}^{k}\right)-f\left(\bm{x}^{k+1}\right), \; \forall k$, 那么

$$
\begin{aligned}
 \frac{1}{2 L} \sum_{k=t_{0}}^{t-1}\left\|\nabla f\left(\bm{x}^{k}\right)\right\|_{2}^{2} & \leq \sum_{k=t_{0}}^{t-1}\left(f\left(\bm{x}^{k}\right)-f\left(\bm{x}^{k+1}\right)\right)=f\left(\bm{x}^{t_{0}}\right)-f\left(\bm{x}^{t}\right) \\ & \leq f\left(\bm{x}^{t_{0}}\right)-f\left(\bm{x}^{*}\right) \\
 \Longrightarrow \quad \min _{t_{0} \leq k<t}\left\|\nabla f\left(\bm{x}^{k}\right)\right\|_{2} & \leq \sqrt{\frac{2 L\left(f\left(\bm{x}^{t_{0}}\right)-f\left(\bm{x}^{*}\right)\right)}{t-t_{0}}}
 \end{aligned}
$$
...
$$
f\left(\bm{x}^{t_{0}}\right)-f\left(\bm{x}^{*}\right) \leq \frac{2 L\left\|\bm{x}^{0}-\bm{x}^{*}\right\|_{2}^{2}}{t_{0}}
$$

令 $t_{0}=t / 2$ 可得 

$$
\min _{t_{0} \leq k<t}\left\|\nabla f\left(\bm{x}^{k}\right)\right\|_{2} \leq \frac{2 L}{\sqrt{t_{0}\left(t-t_{0}\right)}}\left\|\bm{x}^{0}-\bm{x}^{*}\right\|_{2}=\frac{4 L\left\|\bm{x}^{0}-\bm{x}^{*}\right\|_{2}}{t}
$$
