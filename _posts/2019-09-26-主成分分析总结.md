...
$$Cov(\bm{x})=E((\bm{x}-\bm{\mu})(\bm{x}-\bm{\mu})^T)$$
...
看到这个等式差不多要猜测这个很像是 $\bm{X}\bm{X}^{\top}$ 的特征值和特征向量。但是我们并没有说 $\bm{\Lambda}$ 是对角阵，但是已知 $\bm{\Lambda}$ 是对称阵，因此可以正交对角化成对称阵 $\tilde{\Lambda}$：
